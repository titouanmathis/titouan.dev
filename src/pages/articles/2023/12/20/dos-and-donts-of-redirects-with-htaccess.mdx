...
I have spent many hours configuring thousands of redirects when migrating websites, following a redesign or a change of stack. I am sharing some of the do's and don'ts learned along the way and that I would recommend for anyone working on this kind of tasks to follow.
...
To ensure that an exact URL redirects to another exact URL without transmitting information from the original URL, use the `RedirectMatch` direction with a `.*` pattern:
...
Redirect rules in the `.htaccess` file must be configured from the most specific one to the most generic one. The first rule matching the request URL is followed and other rules are not taken into account.
...
To correct this problem, redirect rules can be sorted in reverse alphabetical order:
...
### `Redirect` directives do not take URL parameters into account

If the original URL contains URL parameters (e.g. `?search=keyword`), `Redirect` directives will have no effect on the request.
...
Before deploying your redirect rules in production, it is best to validate them to ensure that they send your users to the correct location.
