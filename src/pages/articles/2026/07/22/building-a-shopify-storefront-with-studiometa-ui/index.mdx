...
[`@studiometa/ui`](https://ui.studiometa.dev) is a library of unstyled, accessible components built on [`@studiometa/js-toolkit`](https://js-toolkit.studiometa.dev), a lightweight progressive-enhancement framework. Both are open-source packages we build and maintain at [ikko](https://ikko.fr). You declare behaviour with `data-` attributes on the HTML your theme already renders, and the components do the wiring. This first article sets up the component model; across the series I build the storefront's interactive pieces one at a time, each on the Liquid your theme already renders.
...
The navigation above swaps a region, shows a loader and keeps the back button working, from a few attributes and no custom fetch code. Next: [the Section Rendering API](/articles/2026/07/29/shopify-section-rendering-api-example/), the technique the rest of the series leans on.
